...
if ($response_code == "0") {
    ...
    if (!$PaymentGatewayRecord) {
        file_put_contents('finalupdate.log', "PaymentGatewayRecord not found for Checkout Request ID: $checkout_req_id\n", FILE_APPEND);
        exit();
    }
    ...
    if (!$userid) {
        file_put_contents('finalupdate.log', "User not found for username: $uname\n", FILE_APPEND);
        exit();
    }
    ...
    if (!$plans) {
        file_put_contents('finalupdate.log', "Plan not found for plan_id: $plan_id\n", FILE_APPEND);
        exit();
    }
