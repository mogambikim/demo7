...
<!-- user-orderPlan --> {if $_c['radius_enable']} {if Lang::arrayCount($radius_hotspot)>0} <div class=" space-y-5">
  <div class="card">
...
<br> {foreach $routers as $router} <div class=" space-y-5">
  <div class="card">
    <header class="card-header">
      <div class="card-title">{$router['name']} | {if $router['description'] != ''} {$router['description']} {/if}</div>
    </header>
    <div class="card-body p-6"> {if count($plans_hotspot)>0} <div class="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-5"> {foreach $plans_hotspot as $plan} {if $router['name'] eq $plan['routers']} <div class="price-table bg-opacity-[0.16] dark:bg-opacity-[0.36] rounded-[6px] p-6 text-slate-900 dark:text-white relative
  ...
  </div> {/if} {if count($plans_pppoe)>0} <div class="grid xl:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-5"> {foreach $plans_pppoe as $plan} {if $router['name'] eq $plan['routers']} <div class="price-table rounded-[6px] shadow-base dark:bg-slate-800 p-6 text-slate-900 dark:text-white relative
